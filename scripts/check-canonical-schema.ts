...
if (migrationFiles.length !== 1 || migrationFiles[0] !== CANONICAL_MIGRATION) {
  console.error(
    `[FAIL] Active migration directory must contain ONLY ${CANONICAL_MIGRATION}. Found: ${migrationFiles.join(", ")}`,
  );
  process.exit(1);
}
...
for (const table of REQUIRED_TABLES) {
  const tableRegex = new RegExp(
    `CREATE TABLE IF NOT EXISTS public\\.${table}\\b|CREATE TABLE public\\.${table}\\b`,
    "i",
  );
...
for (const [parent, child] of ORDER_DEPENDENCIES) {
  ...
  if (parentPos !== undefined && childPos !== undefined) {
    if (parentPos > childPos) {
      console.error(
        `   [FAIL] Ordering violation: Table "${parent}" must be created before dependent table "${child}".`,
      );
      orderErrors++;
    }
  }
...
console.log(
  `   [PASS] All ${ORDER_DEPENDENCIES.length} table creation order dependencies verified.`,
);
...
for (const rpc of REQUIRED_RPCS) {
  if (
    sqlContent.includes(`CREATE OR REPLACE FUNCTION public.${rpc}`) ||
    sqlContent.includes(`CREATE OR REPLACE FUNCTION ${rpc}`)
  ) {
...
if (secDefRegex.test(strippedSql)) {
  console.error(
    `   [FAIL] Found SECURITY DEFINER function missing "SET search_path = public" hardening.`,
  );
  process.exit(1);
}
...
if (fs.existsSync(PROD_SEED_FILE)) {
  ...
  assertProdSeed(
    ...
    "Production seed populates departments",
  );
  assertProdSeed(
    prodSeedContent.includes("public.categories"),
    "Production seed populates categories",
  );
  assertProdSeed(
    prodSeedContent.includes("public.marketplace_configs"),
    "Production seed populates marketplace_configs",
  );
  assertProdSeed(
    !prodSeedContent.includes("auth.users"),
    "Production seed strictly excludes synthetic auth users",
  );
  assertProdSeed(
    !prodSeedContent.includes("public.orders"),
    "Production seed strictly excludes fake staging orders",
  );
} else {
