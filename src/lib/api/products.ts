...
export async function cloneProduct(productId: string): Promise<FullProduct> {
  ...
  const cloneResult = await createProductWithVariants({
    ...
    countryOfOrigin: original.country_of_origin,
    returnEligible: original.return_eligible,
    handlingDays: original.handling_days,
    weightKg: Number(original.weight_kg),
